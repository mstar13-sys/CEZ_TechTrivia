...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if ($act === 'delete_player') {
            ...
            $message  = $userModel->deletePlayer($pid) ? 'Player deleted.' : 'Cannot delete this account.';
            $msgType  = $userModel->deletePlayer($pid) ? 'success' : 'error'; // note: already executed above
        }
        ...
        if ($act === 'add_question') {
            ...
            } elseif ($userModel->addQuestion($text, $diff, $cat, $choices, $correct)) {
        ...
        if ($act === 'edit_question') {
            ...
            if (empty($text) || in_array('', $choices, true)) {
                $message = 'Fill in the question and all 4 choices.'; $msgType = 'error';
            } elseif ($userModel->updateQuestion($qid, $text, $diff, $cat, $choices, $correct)) {
        ...
        if ($act === 'delete_question') {
            $qid     = (int)($_POST['question_id'] ?? 0);
            $message = $userModel->deleteQuestion($qid) ? 'Question deleted.' : 'Failed to delete.';
            $msgType = str_contains($message, 'deleted') ? 'success' : 'error';
        }
    }
}

$players   = $userModel->getAllPlayers();
$questions = $userModel->getAllQuestions();
...
<div class="sidebar">
    ...
    <ul class="nav-menu">
        <li><a href="../views/dashboard.php" style="pointer-events:none;opacity:.4"><span class="nav-icon">🏠</span> Player View</a></li>
    </ul>
